...
function updateInfoBox(map, year) {
    ...
    if (year === "all"){
        data = map.querySourceFeatures('fire-tiles', { sourceLayer: "Fire_perimeters_20002018"})
    } else{
        data = map.querySourceFeatures('fire-tiles', { sourceLayer: "Fire_perimeters_20002018", filter: ['==', ['number', ['get', 'fireyear']], year] });
    }

    let acres = data.map(datum => Number.parseInt(datum.properties.gisacres));

    let sum = 0;

    acres.forEach(datum => sum += datum);
